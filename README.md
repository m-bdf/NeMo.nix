...
This repository is a Nix flake easing the use of NVIDIA's NeMo framework on NixOS.

Run a custom Python script as a background service,
using any NeMo model, on any NeMo version (in theory),
with the optional container runtime of your choice (Podman, systemd-nspawn, or none).

It also exposes all official NeMo images and models currently available,
[their hashes](https://m-bdf.github.io/NeMo.nix) being updated every day
by [a GitHub action](.github/workflows/deploy.yml).
...

{
  inputs = {
    ...
    nemo = {
      ...
      # required until the PR allowing nix2container to work
      # with Nvidia's Container Registry (nvcr.io) is merged
      # (https://github.com/nlewo/nix2container/pull/169)
      inputs.nix2container.url = "github:m-bdf/nix2container";
    };
  };
